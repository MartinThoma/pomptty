...
impl TerminalBackend {
    ...
    fn process_link_action(
        ...
        match link_action {
            ...
            LinkAction::Open => {
                // A real OSC 8 link at the click point wins, regardless of
                // what its visible text looks like; only fall back to the
                // regex-guessed-URL range when there isn't one.
                match terminal.grid()[point.line][point.column].hyperlink() {
                    Some(link) => open::that(link.uri()).unwrap_or_else(|_| {
